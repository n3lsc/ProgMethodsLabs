...
class CurrencyModel:
    ...
    def save(self, filename: str):
        with open(filename, "w", encoding="utf-8") as file:
            for obj in self.data:
                line = (f"{obj.color} {obj.currency_name_1} "
                        f"{obj.currency_name_2} {obj.rate} "
                        f"{obj.date.strftime(DATE_FORMAT)}")
                if getattr(obj, "comment", ""):
                    line = line + f" {obj.comment}"
                file.write(line + "\n")

    # ---------- ADD ----------
    def add(self, color, c1, c2, rate_str, date_str, comment=""):
        try:
            rate = float(rate_str)
            date = datetime.strptime(date_str, DATE_FORMAT).date()
            obj = CurrencyRates(color, c1, c2, rate, date, comment)
            self.data.append(obj)
    ...
    def process_commands(self, filename):
        ...
        def parse_add_args(arg_str: str):
            ...
            if len(parts) not in (5, 6):
                raise ValidationError(f"ADD ожидает 5 или 6 полей: {arg_str}")
            # ensure length 6 for downstream convenience
            if len(parts) == 5:
                parts.append("")
            return parts
